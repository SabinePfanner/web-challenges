...
const onlyTitles = recipes.map((recipe)=>{return recipe.title;}); 

console.log(onlyTitles)// ['Crepes', ...]

const titlesWithDuration = recipes.map((recipe)=>{return recipe.title + " (" + recipe.duration + "min)"});

console.log(titlesWithDuration)// ['Crepes (60min)', ...]

const timePerServing = recipes.map((recipe)=>{return recipe.duration/recipe.servings}); 
...
const allTitlesInOneString = recipes.map((recipe)=>{return recipe.title}).join(", "); 
